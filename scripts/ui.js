...
// 11
FileProgress.prototype.setComplete = function(up, info,WHcontrol) {
  ...
  var url;
  var url2;
  if(WHcontrol.width&&WHcontrol.height)
  {
    var imageView2 = '?imageView2/1/w/'+WHcontrol.width+'/h/'+WHcontrol.height;
  }
  if (res.url) {
    url=res.url
    if(isImage(res.url)){
      url2 = res.url+imageView2;
    }else{
      url2 = res.url
    }
    str = "<div><a href=" + url2 +
      " target='_blank' > " + url2 + "</a></div>" ;
    // str = "<div><strong>Link:</strong><a href=" + res.url +
    //   " target='_blank' > " + res.url + "</a></div>" +
    //   "<div class=hash><strong>Hash:</strong>" + res.hash + "</div>";
  } else {
    var domain = up.getOption('domain');
    url=url2 = domain + encodeURI(res.key)
    if(isImage(url2)){
      url2 = domain + encodeURI(res.key)+imageView2;
    }
    
    var link = domain + res.key;
    str = "<div><a href=" + url2 + " target='_blank' > " +
    url2 + "</a></div>" ;
